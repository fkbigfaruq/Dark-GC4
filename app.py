...
def on_typing(data):
    if "user_id" not in session:
        return
    username = session.get("username") or "someone"
    state = bool(data.get("typing"))
    rid = data.get("room_id")
    to_user = data.get("to_user_id")
    payload = {"username": username, "user_id": session["user_id"], "typing": state}
    if rid:
        try:
            rid = int(rid)
        except Exception:
            return
        payload["room_id"] = rid
        emit("typing", payload, to=f"room:{rid}", include_self=False)
    elif to_user:
        try:
            to_user = int(to_user)
        except Exception:
        ...
        emit("typing", payload, to=f"user:{to_user}")
